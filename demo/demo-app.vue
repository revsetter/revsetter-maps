<template>
  ...
  <div>
    ...
    >
      <template #node="{data,context}">
        <span>
          ...
          />
          <strong>#{{ data.some_id }}</strong> {{ data.label }} &nbsp;
          <a
    ...
    <div>
      ...
      <input id="add_leaf" v-model="inputLeafId" type="number" />
      <button @click="() => tryAddLeaf(inputLeafId, treeData)">+</button>
    </div>
  </div>
</template>
<script>
  ...
  setup() {
    ...
    const deleteNode = (node, tree) => {
      let parent = tree.children
        ? tree.children.find((p) => p.some_id == node.some_id)
        : null;
      if (parent) {
